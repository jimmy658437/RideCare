                ...
                HStack(alignment: .center, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        if let icon = weatherVM.weatherIcon {
                            AsyncImage(
                                url: URL(
                                    string: "https://openweathermap.org/img/wn/\(icon)@4x.png"
                                )
                            ) { image in
            ...
            Group {
                if openWeatherAPIKey.isEmpty {
                    ...
                    VStack(spacing: 28) {
                        ...
                        // 🌟 轉跳 OpenWeather 官網功能
                        Link(destination: URL(string: "https://home.openweathermap.org/api_keys")!) {
                            HStack(spacing: 6) {
                        ...
                        // 儲存按鈕
                        Button {
                            let cleanedKey = tempAPIKey.trimmingCharacters(in: .whitespacesAndNewlines)
                            ...
                            Text("儲存並啟用")
                                ...
                                .padding(.vertical, 14)
                                .background(
                                    tempAPIKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                        ...
                        ScrollView(showsIndicators: false) {
                            VStack(spacing: 24) {
                                VStack(spacing: 4) {
                                    ...
                                    Text("\(String(format: "%.0f", weatherVM.temperature ?? 0))°C")
                                ...
                                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                                    if let feelsLike = weatherVM.feelsLikeTemperature {
                                        WeatherInfoBox(title: "體感溫度", value: "\(String(format: "%.1f", feelsLike))°C", icon: "thermometer")
                                    }
                                    if let humidity = weatherVM.humidity {
                                        WeatherInfoBox(title: "濕度", value: "\(humidity)%", icon: "humidity")
                                    }
                                    if let windSpeed = weatherVM.windSpeed {
                                        WeatherInfoBox(title: "風速", value: "\(String(format: "%.1f", windSpeed)) m/s", icon: "wind")
                                    }
                                    if let pressure = weatherVM.pressure {
                                        WeatherInfoBox(title: "氣壓", value: "\(pressure) hPa", icon: "gauge.with.dots.needle.bottom.100percent")
                                    }
                                    if let visibility = weatherVM.visibility {
                                        WeatherInfoBox(title: "能見度", value: "\(String(format: "%.1f", Double(visibility) / 1000.0)) km", icon: "eye")
                                    }
                                    if let cloudiness = weatherVM.cloudiness {
                                        WeatherInfoBox(title: "雲量", value: "\(cloudiness)%", icon: "cloud")
                                    }
                                }
                                .padding(.horizontal)
                                ...
                                HStack(spacing: 16) {
                                    if let sunrise = weatherVM.sunrise {
                                        WeatherInfoBox(title: "日出時間", value: sunrise.formatted(date: .omitted, time: .shortened), icon: "sunrise.fill")
                                    }
                                    if let sunset = weatherVM.sunset {
                                        WeatherInfoBox(title: "日落時間", value: sunset.formatted(date: .omitted, time: .shortened), icon: "sunset.fill")
                                    }
                                }
                                .padding(.horizontal)
                                .padding(.bottom, 30)
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Button {
                        ...
                        Image(systemName: "xmark.circle.fill")
                            ...
                            .foregroundStyle(openWeatherAPIKey.isEmpty ? Color.primary.opacity(0.6) : .white)
                            .font(.title2)
                    }
                }
            }
